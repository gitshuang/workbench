...
const defaultState = {
  ...
  pinType: false, // NoDictionarypinNoDictionary
  pinDisplay: false, // pinNoDictionary
  widthBrm: true,
};
...
const reducer = handleActions({
  [addBrm]: (state, { payload: data }) => {
    ...
    if (nowBrmLast.length > 0 && data.name === nowBrmLast[nowBrmLast.length - 1].name) {
      ...
      topBrm = [...nowBrmLast, data];// NoDictionary
  ...
  [popBrm]: (state, { payload: data }) => {
    ...
    if (index === -1) {
      ...
      const popBrmName = stateBrm[stateBrm.length - 1][index].name;// NoDictionarybrmNoDictionary
      let item;
