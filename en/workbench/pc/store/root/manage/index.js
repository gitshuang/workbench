...
const reducer = handleActions({
  ...
  [addFolder]: (state, {
    ...
    const newdefaultFolder = {
      type: 2,
      widgetName: newFolderName, // "NoDictionary"+_fileName,
      children: [],
    };
    const temp = {
      ...newdefaultFolder,
  ...
  [moveService]: (state, {
    ...
    const sourceData = preParentId && findById(manageAllList, preParentId); // NoDictionary NoDictionary
    const targetData = parentId && findById(manageAllList, parentId); // NoDictionary NoDictionary
    ...
    if (preType === 3 && afterType === 2 && timeFlag) {
      // 从外面拖入文件夹里面
      sourceData.children.splice(sourceData.children.indexOf(itemIn), 1); // NoDictionary
      if (preParentId !== afterId) {
        itemIn.parentId = afterId;
      }
      itemAfter.children.push(itemIn); // NoDictionary
    } else if (
      ...
      if (ifIntoFile == 'left') {
        targetData.children.splice(targetData.children.indexOf(itemAfter), 0, itemIn); // NoDictionary
      } else {
        targetData.children.splice(targetData.children.indexOf(itemAfter) + 1, 0, itemIn); // NoDictionary
      }
      ...
      targetData.children.splice(targetData.children.length, 0, itemIn); // NoDictionary
    } else {
