...
class CreateTeamContent extends Component {
  ...
  esitTime = (time) => {
    ...
    return "還有" + Math.ceil(timeDiff) + "日過期"
  }
  // 续费按钮
  esitXufei = (time) => {
    // 判断到期时间没有或者... 直接不继续
                ...
                return (
                  <li className="um-box" key={index}>
                    ...
                    <div>
                      ...
                      {
                        item.expired ? <p>到期時間：{this.handelTime(item.expired)}</p> : ""
                      }
                    </div>
                    <div>
                    ...
                    <div className="um-bf1 tr">
                      {
                        this.esitXufei(item.expired) ? <Button onClick={() => { this.openXufei(item.applicationId) }}>續費</Button> : null
                      }
      ...
      <div className={box3}>
        <div className={memberTit}>
          ...
          <div className={memberBtns}>
            <Checkbox colors="info" onChange={this.changeCheck}>只顯示管理員</Checkbox>
            {
              _invitePermission ? <ButtonBrand onClick={this.inviteMember}>邀請成員</ButtonBrand> : null
            }
          </div>
        </div>

        <div className={table_title}>
          <div>當前人数{userList.totalElements}人</div>
                  ...
                  <li key={index}>
                    ...
                    <div className={`${option_sele} um-bf1  um-box-vc `}>
                      ...
                      >
                        ...
                        <Option value="deleteMember">移除成員</Option>
                      </Select>
                    </div>
                  </li>
                )
      ...
      <div className={wrap}>
        <div className={header}>
          ...
          <div className="um-box um-box-center">
            <div>
              <Button onClick={this.openUpgradeModal}>升級為企業</Button>
            </div>
            <div>
              {/* <Dropdown
                trigger={['click']}
              ...
              >
                <Button className="um-box-vc um-box-center">相關操作<Icon type="pull-down" /></Button>
              </Dropdown> */}
        ...
        <div className={content}>
          <Tabs
            ...
            defaultActiveKey="1"
          >
            <TabPane tab='基礎設置' key="1">
              <div >
                <CreateTeam />
              </div>
            </TabPane>
            <TabPane tab='團隊應用' key="2">
              {this.teamApplication()}
            </TabPane>
            <TabPane tab='團隊成員' key="3">
              {this.teamMember()}
            </TabPane>
          </Tabs>
        </div>
