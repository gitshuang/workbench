      ...
      <div className={wrap}>
        ...
        <div className={footer}>
          <div className={now ? `${process_loading_content} ${opacityShow}` : process_loading_content}>
            <Progress loadingCallBack={this.loadingCallBack} startFlag={now} successFunc={this.successLoading} loadingDesc={'正在配置團隊資訊…'} />
          </div>
          <Button className={`${now ? opacityHidden : ''} ${submit_class}`} onClick={this.create} disabled={disabled} >創建</Button>
        </div>
      </div>
    )
  }
