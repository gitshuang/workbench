...
class QuickServiceContainer extends Component {
  ...
  componentDidMount() {
    ...
    let b = true;
    b = serviceList && serviceList.length <= 12 ? true : false;
    this.setState({
      openAllstate: b
    })
  }
        ...
        </div>
        {
          this.state.openAllstate ? (
            <div className={link_cont} onClick={this.openAllAppList}>
              <a>$i18n{index.js0}$i18n-end</a>
            </div>
          ) : (
              <div className={serviceBtn}>
                <button className={btn} onClick={this.onClickScroll}>$i18n{index.js1}$i18n-end</button>
              </div>
            )
        }
