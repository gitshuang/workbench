...
class searchOther extends Component {
  ...
  paginationNumSelect = (id,dataNum) =>{
    ...
    const reg = new RegExp("$i18n{index.js0}$i18n-end\/$i18n{index.js1}$i18n-end","g");
  ...
  Datetimechange(ts){
    ...
    if (dDays > 0 && dDays < 4) {
      ts = `${dDays}$i18n{index.js2}$i18n-end`;
    } else if (dDays <= 0 && dHours > 0) {
      ts = `${dHours}$i18n{index.js3}$i18n-end`;
    } else if (dDays <= 0 && dHours <= 0 && dMinutes > 0) {
      ts = `${dMinutes}$i18n{index.js4}$i18n-end`;
    } else if (dSeconds < 60) {
      ts = '$i18n{index.js5}$i18n-end';
    } else if (dDays > 3) {
      ts = new Date(parseInt(ts, 10)).toLocaleString().replace(/$i18n{index.js6}$i18n-endg, '-').replace(/$i18n{index.js7}$i18n-endg, ' ');
    }
    return ts
  }
      ...
      <div className={bg+" um-content um-vbox"}>
        <div className={bg_wrap+" um-content um-vbox"}>
          <div className={`${wrap} ${clearfix} um-content um-vbox`}>
            <div>$i18n{index.js8}$i18n-end{dataList.totalElements}$i18n{index.js9}$i18n-end</div>
