      ...
      <div className={wrap}>
        ...
        <div className={footer}>
          <div className={now ? `${process_loading_content} ${opacityShow}` : process_loading_content}>
            <Progress loadingCallBack={this.loadingCallBack} startFlag={now} successFunc={this.successLoading} loadingDesc={'$i18n{index.js5}$i18n-end…'} />
          </div>
          <Button className={`${now ? opacityHidden : ''} ${submit_class}`} onClick={this.create} disabled={disabled} >$i18n{index.js6}$i18n-end</Button>
        </div>
      </div>
    )
  }
