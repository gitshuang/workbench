        ...
        <div className={bg_wrap + " um-content um-vbox"}>
          <div className={`${wrap} ${clearfix} um-content um-vbox`}>
            <div className={searchPanel}>
              <FormControl className={serviceSearch} placeholder="$i18n{index.js1}$i18n-end" value={value} onKeyDown={this.onKeyup} onChange={this.inputOnChange} />
              <div className={search_icon_con}>
                <span>|</span>
                <Icon type="search" className={ufSearch} onClick={() => { this.btnSearch() }}></Icon>
                <span className={search_tit} onClick={() => { this.btnSearch() }}>$i18n{index.js2}$i18n-end</span>
              </div>
              {_appType ? <ButtonBrand className={openMarketBtn} onClick={this.openMarket} >$i18n{index.js3}$i18n-end</ButtonBrand> : null}
            </div>
