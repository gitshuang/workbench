...
class MoveToGroup extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      // 新分组名
      newGroupName: '$i18n{index.js0}$i18n-end',
      // 是否是打开新的分组
      inAddGroup: false,
      // 路径
      way: '',
  ...
  addGroup = () => {
    ...
    const newGroupName = avoidSameName(nameArr, '$i18n{index.js1}$i18n-end');
    this.setState({
      inAddGroup: true,
      newGroupName,
    });
  ...
  render() {
    ...
    const content = (
      <div className={container}>
        <div className={title}>
          {caller}$i18n{index.js2}$i18n-end{way}
        </div>
        ...
        <div className={`${footer} um-box-justify`} style={{ overflow: 'hidden' }}>
          {
            onAddGroup ? (
              <div>
                <Button style={{ float: 'left' }} onClick={this.addGroup} disabled={inAddGroup}>$i18n{index.js3}$i18n-end</Button>
              </div>
            ) : null
          }
